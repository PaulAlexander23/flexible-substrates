function [c, x, y] = plot_neutral_curve_single_contour(method,k,R,cotbeta,S,AD,AT,AB,AK,AI,modes,varargin)
    %PLOT_NEUTRAL_CURVE
    if nargin < 8
        modes = 1;
    end
    ...
    save("recent_data",'x','y','c')

    contour(x, y, prod(c.*(-1*ones(1,1,modes)).^(1+1:modes), 3), [0,0], 'k');
